...
# Optimized for Doom/Wolfenstein: 320x200x8bit @ ~25fps
...
# Protocol receiver - MODIFIED for 8-bit palette indices
class ProtoSPIRx(LiteXModule):
    def __init__(self, spi_source, row_fifo_sink):
        ...
        pixels_out = Signal(10)  # Changed from words_out to pixels_out
        ...
        fsm.act("PAYLOAD",
            If(pixels_out == GAME_WIDTH,  # 320 pixels per row
                ...
                If(rx.valid & dst.ready,
                    dst.valid.eq(1),
                    dst.data.eq(rx.data),  # Direct 8-bit palette index
...
# Row writer to SDRAM - MODIFIED for 8-bit data
class WB8RowWriter(LiteXModule):
    def __init__(self, base_words, stride_words):
        self.src = src = stream.Endpoint([("data", 8)])  # 8-bit palette indices
...
# Line fetcher from SDRAM - MODIFIED for 8-bit data
class WB8LineFetchToBRAM(LiteXModule):
    def __init__(self, base_words, stride_words):
        ...
        self.out_dat = Signal(8)   # 8-bit palette index
        self.done = Signal()
...
# Frame clear - MODIFIED for smaller game resolution
...
# Main SoC - MODIFIED for Doom optimization
class Top(SoCCore):
    def __init__(self, sys_clk_freq=SYS_CLK_FREQ, **kwargs):
        ...
        try:
            SoCCore.__init__(self, platform, clk_freq=sys_clk_freq, ident="SPI->SDRAM->HDMI-Doom", **kwargs)
        except TypeError:
            SoCCore.__init__(self, platform, sys_clk_freq, ident="SPI->SDRAM->HDMI-Doom", **kwargs)
        ...
        # MODIFIED: 8-bit pixels, 320 pixels per line
        ...
        # Line buffers - MODIFIED for 8-bit palette and 320 pixels
        memA = Memory(8, GAME_WIDTH)  # 8-bit palette indices
        ...
        # Initialize basic Doom-style palette
        def doom_palette():
            palette = []
            for i in range(256):
                if i < 16:      # Grays (walls)
                    gray = (i << 4) | i
                    palette.append((gray << 16) | (gray << 8) | gray)
                elif i < 32:    # Browns (walls/floors)
                    r = 128 + (i - 16) * 4
                    g = 64 + (i - 16) * 2  
                    b = 32
                    palette.append((r << 16) | (g << 8) | b)
                elif i < 48:    # Reds (blood/enemies)
                    r = 128 + (i - 32) * 8
                    g = (i - 32) * 2
                    b = (i - 32) * 1
                    palette.append((r << 16) | (g << 8) | b)
                else:           # General colors
                    # Generate varied colors for sprites/textures
                    r = (i * 123) & 0xFF
                    g = (i * 179) & 0xFF  
                    b = (i * 251) & 0xFF
                    palette.append((r << 16) | (g << 8) | b)
        ...
        # Palette memory - 256 colors × 24-bit RGB (initialized with palette data)
        palette_mem = Memory(24, 256, init=doom_palette())
        self.specials += palette_mem
        palette_port = palette_mem.get_port(clock_domain="hdmi")
        self.specials += palette_port
        ...
        rd_palette_idx = Signal(8)
        self.comb += rd_palette_idx.eq(Mux(read_bank, rdB.dat_r, rdA.dat_r))
        
        # Palette lookup
        self.comb += palette_port.adr.eq(rd_palette_idx)
        rgb_out = Signal(24)
        self.comb += rgb_out.eq(palette_port.dat_r)
        ...
        self.comb += [
            ...
            self.videophy.sink.r.eq(Mux(display_ready & valid_game_area, rgb_out[16:24], 0)),
            self.videophy.sink.g.eq(Mux(display_ready & valid_game_area, rgb_out[8:16], 0)),
            self.videophy.sink.b.eq(Mux(display_ready & valid_game_area, rgb_out[0:8], 0)),
        ]
        ...
        self.submodules.row_fifo = SyncFIFO([("data",8)], depth=GAME_WIDTH)  # 8-bit data
