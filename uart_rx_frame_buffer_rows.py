...
#
# No CPU. Two Wishbone DMAs:
#  - Row writer: UART row -> back SDRAM frame
#  - Line fetcher: front SDRAM frame -> BRAM line buffer (for HDMI scanout)
...
# Use your platform module (local or official)
try:
...
# ---------------- Clocks / UART ----------------
SYS_CLK_FREQ = int(48e6)    # stable and UART-friendly
UART_BAUD    = 6_000_000    # fast enough for rows with backpressure


# -------- Adapter: map discrete SDRAM pins to canonical names -----------
class _SDRAMPads:
    """
    Tries to request discrete SDRAM pins:
    ...
    If these don't exist, caller should fall back to platform.request("sdram").
    """
    def __init__(self, platform):
        ...
        self.we_n  = platform.request("O_sdram_wen_n")   # board names "wen", litedram expects "we_n"
        self.dm    = platform.request("O_sdram_dqm")     # width 2
        self.a     = platform.request("O_sdram_addr")    # width 13 (A0..A12)
...
class ProtoUARTRx(LiteXModule):
    ...
    Outputs (sys domain):
      ...
      - sof_pulse / eof_pulse: 1-cycle pulses when AA44 / AA66 seen
    """
    def __init__(self, uart_source, row_fifo_sink):
        ...
        # Accept low byte immediately; gate high byte by FIFO ready so we never overflow
        fsm.act("PAYLOAD",
        ...
        fsm.act("ROW_DONE_CLR",
            NextValue(self.row_done, 0),
            NextState("WAIT_A")
        )
...
class WB32RowWriter(LiteXModule):
    """
    Pulls 640x16b from src and writes 320x32b words to SDRAM at:
      base_words + y*stride_words + widx
    """
...
class WB32LineFetchToBRAM(LiteXModule):
    ...
    Out signals must be connected to the selected BRAM write port.
    """
    def __init__(self, base_words, stride_words):
        self.wb     = wishbone.Interface()
        self.start  = Signal()
        ...
        fsm.act("RD_ISSUE",
            ...
            If(self.wb.ack,
                NextValue(data_lat, self.wb.dat_r),
                NextState("WR_LO")
            )
        )
        fsm.act("WR_LO",
        ...
        fsm.act("WR_HI",
            self.out_we.eq(1),
            self.out_adr.eq(x),
            self.out_dat.eq(data_lat[16:32]),
            NextValue(x, x + 1),
            If(word_idx == (H_VISIBLE//2 - 1),
                NextState("DONE")
            ).Else(
...
class Top(SoCCore):
    def __init__(self, sys_clk_freq=SYS_CLK_FREQ, **kwargs):
        ...
        # SDRAM pads: try discrete mapping first, else standard "sdram" record
        try:
            ...
            need = [("a",13),("ba",2),("dq",16),("dm",2),("clk",1),("cke",1),("cs_n",1),("ras_n",1),("cas_n",1),("we_n",1)]
            probs = [n for (n,w) in need if not hasattr(sdram_pads,n) or _w(getattr(sdram_pads,n))!=w]
            if probs: 
                raise ValueError("Discrete SDRAM pins not matching widths: %s" % probs)
        except Exception:
            sdram_pads = platform.request("sdram", 0)
        ...
        # Swap read bank on each end-of-line (DE falling edge)
        de_d = Signal()
        self.sync.hdmi += de_d.eq(self.tmg.de)
        de_fall = Signal()
        self.comb += de_fall.eq(de_d & ~self.tmg.de)
        self.sync.hdmi += If(de_fall, read_bank.eq(~read_bank))

        # Next line index in HDMI -> SYS for prefetch
        y_next_hdmi = Signal(16)
        self.sync.hdmi += If(de_fall,
        ...
        # Write-bank (SYS) follows the opposite of the HDMI read bank (safely synced)
        ...
        # Kick a new line fetch once per line
        de_fall_sys   = Signal(); self.specials += MultiReg(de_fall, de_fall_sys, "sys")
        de_fall_sys_d = Signal(); self.sync += de_fall_sys_d.eq(de_fall_sys)
        line_kick     = Signal(); self.comb += line_kick.eq(de_fall_sys & ~de_fall_sys_d)

        self.comb += [
            self.line_fetch.start.eq(line_kick),
        ...
        self.sync += [
            If(k_edge & ~self.ack_fifo.sink.valid,
                self.ack_fifo.sink.valid.eq(1),
                self.ack_fifo.sink.data.eq(ord('K'))
            ).Elif(self.ack_fifo.sink.valid & self.ack_fifo.sink.ready,
                self.ack_fifo.sink.valid.eq(0)
            )
        ]
        ...
        # 'F' one-shot when we actually swap
        swap_prev = Signal()
        f_edge = Signal()
        self.sync += [
        ...
        self.sync += [
            If(f_edge & ~self.ack_fifo.sink.valid,
                self.ack_fifo.sink.valid.eq(1),
                self.ack_fifo.sink.data.eq(ord('F'))
            ).Elif(self.ack_fifo.sink.valid & self.ack_fifo.sink.ready,
                self.ack_fifo.sink.valid.eq(0)
            )
        ]
